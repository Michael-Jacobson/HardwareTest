...
*
*	Copyright(c) 2003 - 2022 by Adaptive Digital Technologies, Inc.
*	All Rights Reserved
*
*
********************************************************************/
...
#define ENCODE_CHAN_SIZE    546
#define ENCODE_SCRATCH_SIZE 1300
...
typedef struct
{
  	ADT_Int64 encodeData[(ENCODE_CHAN_SIZE+1)/2];
} G729ENC_ADT_Instance_t;

typedef struct 
{
    ADT_Int64 encodeScratch[(ENCODE_SCRATCH_SIZE+1)/2];
} G729ENC_ADT_Scratch_t;

typedef struct 
{
    ADT_Int64 decodeData[(DECODE_CHAN_SIZE+1)/2];
} G729DEC_ADT_Instance_t;

typedef struct 
{
    ADT_Int64 decodeScratch[(DECODE_SCRATCH_SIZE+1)/2];
